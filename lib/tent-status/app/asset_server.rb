    ...
    class AssetServer < Middleware
      ...
      module SprocketsHelpers
        ...
        def asset_path(source, options = {})
          ...
          "#{TentStatus.settings[:asset_root]}/#{asset.digest_path}"
        end
      end
      ...
      def self.sprockets_environment
        @environment ||= begin
          environment = Sprockets::Environment.new do |env|
            ...
            env.cache = Sprockets::Cache::FileStore.new(TentStatus.settings[:asset_cache_dir]) if TentStatus.settings[:asset_cache_dir]
          end
